...
var collection = require('../models/trackinglistmodel.js');
describe('models', function() {
    ...
    it('should query the tracking list for user', function(done) {
        ...
        collection.qeuryTrackingList(queryParams, function(err, result) {
            should.not.exist(err);
//            should.exist(result);
...
describe('controllers', function() {
    ...
    it('should replace the tracking list for user', function(done) {
        var postData = {user: 'user',
            trackinglist: ['user','user2', 'user1', 'user3', 'user4', 'user5', 'user7', 'user10']};
        request
                .post('/trackinglist')
                .send(postData)
                .expect(200)
...
//            console.dir(res.body);
            res.body.result.trackinglist[0].should.equal('user');
            done();
        });
    });
    ...
    it('should get the tracking list for user', function(done) {
        request
            ...
            res.body.result.user.should.equal('user');
            res.body.result.trackinglist[0].should.equal('user');
            res.body.result.trackinglist[1].should.equal('user2');
            done();
        });
    });
