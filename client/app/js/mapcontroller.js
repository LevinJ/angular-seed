...
angular.module('myApp.controllers')
        .controller('RouteCtrl', ['$scope', 'TrasmitPosition', 'Auth', 'TrasmitTrackingList', function($scope, TrasmitPosition, Auth, TrasmitTrackingList) {

        $scope.update = function(currentusertracked) {
            var queryParams = {};
            queryParams.username = currentusertracked;
...
//                    queryParams.endtime = new Date(2015, 4, 1);
            ...
            queryParams.starttime.setDate(queryParams.endtime.getDate() - 1);
            $scope.positions = [];
        ...
        $scope.trackingusers = [];
        $scope.currentusertracked = Auth.user.username;
        var queryParams = {};
        queryParams.user = Auth.user.username;
        TrasmitTrackingList.get(queryParams)
                .then(function(value) {
            $scope.trackingusers = value.data.result.trackinglist;
        });
...
//                     });
        ...
        .controller('HomeCtrl', ['$scope', 'GetPosition', 'BaiduMap', function($scope, GetPosition, BaiduMap) {
        GetPosition.get()
                .then(function(value) {
            BaiduMap.showBase();
            return value;
                ...
                .then(function(value) {
            return BaiduMap.showMarker(value);
        })
                . catch (function(reason) {
            console.log(reason);
        });
